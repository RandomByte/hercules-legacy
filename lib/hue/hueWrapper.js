...
class HueWrapper {
	constructor(oParams) {
		this.oHue = new Hue({
			sHueConfigPath: oParams.sHueConfigPath
		});
		this.mHandledLights = {};
		this.mHandledGroups = {};
	}
	...
	handleGroup(sRoom, bOn) {
		...
		return this.oHue.getClient().groups.getById(iGroupId).then(function(oGroup) {
			...
			if (oGroup.anyOn && !oMetaGroup) {
				...
				return;
			} else if (!bOn && !oGroup.allOn) {
				// We handle or not handle it, but it's already off anyway
				// 	-> nothing to do
				return;
			} else if (oMetaGroup && (bOn && oGroup.allOn)) {
				// We handle it and it's already on
				//	-> nothing to do
