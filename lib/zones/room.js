...
class Room {
	...
	updateLight() {
		if (this._iUpdateLightTimeout) {
			clearTimeout(this._iUpdateLightTimeout);
		}

		this._iUpdateLightTimeout = setTimeout(this._updateLight.bind(this));
	}
	...
	_updateLight() {
		...
		if (bMotion && iPresenceScore === 1) {
			debug("[%s: Action]: Unconfirmed sign of presence -> turning on light for 10sec awaiting confirmation",
				this._sName);
			if (!this._iUnconfirmedPresenceTimeout) {
				this._iUnconfirmedPresenceTimeout = setTimeout(function() {
					this._iUnconfirmedPresenceTimeout = null;
					debug("[%s Action:]: Presence could not be confirmed in time - turning light off", this._sName);
					this._switchLight(false);
				}.bind(this), 10000);
			}
		} else if (this._iUnconfirmedPresenceTimeout && bMotion && iPresenceScore > 1) {
			// Confirmed presence while we were waiting for confirmation -> cancel timeout
			clearTimeout(this._iUnconfirmedPresenceTimeout);
		}
