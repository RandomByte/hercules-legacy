...
class RestServer {
	...
	_initExpress(iHttpPort) {
		...
		app.post("/api/status", function(req, res) {
			if (req.body.bHandleSensorInput !== undefined) {
				that.oSystemState.setHandleSensorInput(req.body.bHandleSensorInput);
			}
			res.send(that._getSystemStatus());
		});
	...
	_getSystemStatus() {
		return {
			bHandleSensorInput: this.oSystemState.getHandleSensorInput()
		};
	}
}
